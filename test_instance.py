...
if __name__ == '__main__':
    ...
    # 测试 @根据用户名 查询所属角色
    roels = roleservice.find_role_by_user_id(3)
    for role in roels:
        if role is not None:
            print(role)
            
    # 测试 @根据角色名 查询用户       
    users = roleservice.find_users_by_role_name('SAAA')
    for user in users:
        if user is not None:
           print(user)
    ...
    perm = db.session.query(Permission).filter(Permission.name=='系统配置管理').first()
